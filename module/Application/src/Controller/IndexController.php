...
use Application\Entity\FilteredMovie;
...
class IndexController extends AbstractActionController
{
    ...
    private function createSearchView(): JsonModel
    {
        ...
        $this->clearTempTable();
        $this->saveTemporaryMovies($this->viewModel->getVariable('movies'));
    ...
    private function getAllSearchHistory(): array
    {
        ...
        // HERE WE USE THE SAME METHOD AS WITH FINDING BY GENRE OR LANGUAGE, BUT WE SEARCH FOR THE MOVIE BY NAME
    ...
    // THIS FUNCTION SAVES VALUES OF ALL FILTERED ENTITIES TO filtered_movies TABLE
    public function saveTemporaryMovies(array $movies)
    {
        $filteredMovieRepository = $this->entityManager->getRepository(FilteredMovie::class);

        foreach ($movies as $movie) {

            $movieItem = $this->entityManager
            ->getRepository(Movie::class)
            ->findBy(['name' => $movie['name']]);

            $filteredMovie = new FilteredMovie;
            $filteredMovie->setName($movie['name']);
            $this->entityManager->persist($filteredMovie);
        }
        $this->entityManager->flush();

    }

    // THIS FUNCTION DELETES THE PREVIOUS VALUES OF filtered_movies TABLE
    public function clearTempTable(): int
    {
        return $qb = $this->entityManager->createQueryBuilder()
            ->delete()
            ->from(FilteredMovie::class, 'm')
            ->getQuery()
            ->getResult();
    }
}
